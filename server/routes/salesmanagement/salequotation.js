...
router.get("/", async (req, res) => {
    try {
        const sales = await pool.query('SELECT * FROM Sales_Quotation_Invoice WHERE "is_deleted"=false');
...
router.post('/', async (req, res) => {
    ...
    try {
        const create = await pool.query("INSERT INTO Sales_Quotation_Invoice(Document_No, DATE, BASE, BASE_REF_NO, Customer, Net_Amount, Entry_User, Entry_Date, Status) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *",
...
router.put('/:id', async (req, res) => {
    ...
    const { Document_No, DATE, BASE, BASE_REF_NO, Customer, Net_Amount, Entry_User, Entry_Date, Status } = req.body;
    try {
        const update = await pool.query("UPDATE Sales_Quotation_Invoice SET Document_No=$1, DATE=$2, BASE=$3, BASE_REF_NO=$4, Customer=$5, Net_Amount=$6, Entry_User=$7, Entry_Date=$8, Status=$9 WHERE id=$10 AND is_deleted=false", [Document_No, DATE, BASE, BASE_REF_NO, Customer, Net_Amount, Entry_User, Entry_Date, Status, id]);
