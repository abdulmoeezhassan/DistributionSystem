...
router.get('/', async (req, res) => {
    try {
        const getCustomer = await pool.query('SELECT id,Supplier_Number,Supplier_Type,Name,Phone,City,Entry_Date FROM Customer WHERE "is_deleted"=false');
...
Credit_Days,
Lincense_Number,
Lincense_Expiry FROM Customer WHERE  "is_deleted"=false AND id=$1
`, [id]);
