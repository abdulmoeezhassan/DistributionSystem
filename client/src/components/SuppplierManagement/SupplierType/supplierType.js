...
const Salesinvoice = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:4000/SupplierType/${id}`);
      if (response) {
  ...
  const getData = async () => {
    try {
      const GetDATA = await axios.get("http://localhost:4000/SupplierType");
      if (GetDATA) {
      ...
      {showAddNew && (
        <div className="popup-container" >
          <div className="popup-contentt">
            <DesComp head="Supplier Type" apiendpoint="http://localhost:4000/SupplierType" id={id} />
