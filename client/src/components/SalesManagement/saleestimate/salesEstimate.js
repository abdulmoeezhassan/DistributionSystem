...
const Salesinvoice = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:4000/SalesEstimateInvoice");
        if (response) {
  ...
  const handleDelete = async(id) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:4000/SalesEstimateInvoice/${id}`);
  ...
  return (
    <div>
      <div className="headersales">
        <h5>Sales Estimate</h5>
        <button onClick={handleAddNewClick}>+|Add New</button>
      </div>
      {showAddNew && (
        <div className="popup-container" >
          <div className="popup-contenttt">
            <AddNew head="Sales Estimate" apiendpoint="http://localhost:4000/SalesEstimateInvoice" id={id}/>
              ...
              getData.map((item, index) => (
                <tr key={index}>
                  ...
                  <img src={DeleteBtn} alt="Delete Button" className="bbttn1" onClick={() => handleDelete(item.id)} />
                    <img src={EditBtn}  alt="Edit Button" className="bbttn2" onClick={() => handleUpdate(item.id)}/>
                    <img  src={PrintBtn} alt="Print Button" className="bbttn3" onClick={() => handlePrint(item.id)}/>
                  </td>
                </tr>
              ))
            ) : (
