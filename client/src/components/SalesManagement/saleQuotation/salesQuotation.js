...
const Salesinvoice = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:4000/SalesQuotationInvoice");
        if (response) {
  ...
  const handleDelete = async(id) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:4000/SalesQuotationInvoice/${id}`);
      ...
      {showAddNew && (
        <div className="popup-container" >
          <div className="popup-contenttt">
            <AddNew head="Sales Quotation" apiendpoint="http://localhost:4000/SalesQuotationInvoice" id={id}/>
