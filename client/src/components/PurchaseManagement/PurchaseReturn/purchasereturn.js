...
const Salesinvoice = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:4000/PurchaseReturn");
        if (response) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:4000/PurchaseReturn/${id}`);
      ...
      {showAddNew && (
        <div className="popup-container" >
          <div className="popup-contenttt">
            <AddNew head="Purchase Return" apiendpoint="http://localhost:4000/PurchaseReturn" id={id} />
