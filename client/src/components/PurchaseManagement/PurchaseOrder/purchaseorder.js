...
const Salesinvoice = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:4000/PurchaseOrder");
        if (response) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:4000/PurchaseOrder/${id}`);
      if (response) {
      ...
      {showAddNew && (
        <div className="popup-container" >
          <div className="popup-contenttt">
            <AddNew head="Purchase Order" apiendpoint="http://localhost:4000/PurchaseOrder" id={id} />
