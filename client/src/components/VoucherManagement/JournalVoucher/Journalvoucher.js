...
const Salesinvoice = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:4000/JournalVoucher");
        if (response) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:4000/JournalVoucher/${id}`);
      ...
      {showAddNew && (
        <div className="popup-container" >
          <div className="popup-contenttt">
            <AddNew head="Journal Voucher" apiendpoint="http://localhost:4000/JournalVoucher" id={id} />
