...
const Salesinvoice = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:4000/CustomerType/${id}`);
      if (response) {
  ...
  const getData = async () => {
    try {
      const GetDATA = await axios.get("http://localhost:4000/CustomerType");
      if (GetDATA) {
      ...
      {showAddNew && (
        <div className="popup-container" >
          <div className="popup-contentt">
            <DesComp head="Customer Type" apiendpoint="http://localhost:4000/CustomerType" id={id} />
